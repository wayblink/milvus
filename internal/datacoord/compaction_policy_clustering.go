...
func newClusteringCompactionPolicy(meta *meta, view *FullViews, allocator allocator, compactionHandler compactionPlanContext, handler Handler) *clusteringCompactionPolicy {
	interval := Params.DataCoordCfg.ClusteringCompactionTriggerInterval.GetAsDuration(time.Second)
	ticker := time.NewTicker(interval)
	return &clusteringCompactionPolicy{meta: meta, view: view, ticker: ticker, allocator: allocator, compactionHandler: compactionHandler, handler: handler}
}
...
//
//func (policy *clusteringCompactionPolicy) Ticker() *time.Ticker {
//	return policy.ticker
//}
//
//func (policy *clusteringCompactionPolicy) Stop() {
//	policy.ticker.Stop()
//}

func (policy *clusteringCompactionPolicy) Trigger() (map[CompactionTriggerType][]CompactionView, error) {
	// support config hot refresh
	//defer policy.ticker.Reset(Params.DataCoordCfg.ClusteringCompactionTriggerInterval.GetAsDuration(time.Second))
...
func (policy *clusteringCompactionPolicy) collectionIsClusteringCompacting(collectionID UniqueID) (bool, int64) {
	...
	if len(tasks) > 0 {
		cTasks := make([]*datapb.CompactionTask, 0)
