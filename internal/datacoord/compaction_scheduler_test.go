...
func (s *SchedulerSuite) TestScheduleParallelTaskFull() {
	...
	tests := []struct {
		description string
		tasks       []*mixCompactionTask
		expectedOut []UniqueID // planID
	}{
		{"with L0 tasks", []*mixCompactionTask{
	...
	for _, test := range tests {
		s.Run(test.description, func() {
			...
			s.Equal(test.expectedOut, lo.Map(gotTasks, func(t *mixCompactionTask, _ int) int64 {
...
func (s *SchedulerSuite) TestScheduleNodeWith1ParallelTask() {
	...
	tests := []struct {
		...
		{"with L0 tasks diff channel", []*mixCompactionTask{
		...
		{"with L0 tasks same channel", []*mixCompactionTask{
...
func (s *SchedulerSuite) TestFinish() {
	...
	s.Run("finish from queuingTasks", func() {
		...
		s.scheduler.Submit(&mixCompactionTask{plan: plan, dataNodeID: datanodeID})
