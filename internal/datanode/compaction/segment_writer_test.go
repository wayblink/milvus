...
func (s *SegmentWriterSuite) TestWriteFailed() {
	...
	s.Run("get bm25 field failed", func() {
		...
		writer, err := NewSegmentWriter(schema, 1024, 1, partitionID, collectionID, []int64{1000})
	...
	s.Run("parse bm25 field data failed", func() {
		...
		writer, err := NewSegmentWriter(schema, 1024, 1, partitionID, collectionID, []int64{101})
		s.Require().NoError(err)
