...
func (t *level2CompactionTask) compact() (*datapb.CompactionPlanResult, error) {
	...
	// 0, analyze
	// t.analyzeStep(ctxTimeout, pkField)
...
func (t *level2CompactionTask) analyzeStep(ctx context.Context, pkField *schemapb.FieldSchema) error {
	flightSegments := t.plan.GetSegmentBinlogs()
	for _, segment := range flightSegments {
		analyzeResult, err := t.analyze(ctx, pkField, segment)
		if err != nil {
			return err
		}
		for k, v := range analyzeResult {
			log.Info("analze result", zap.Any("key", k), zap.Int64("count", v))
		}
	}
	return nil
}

func (t *level2CompactionTask) analyze(
	ctx context.Context,
