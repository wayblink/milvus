...
func getVectorFieldFromSchema(schema *schemapb.CollectionSchema) (*VectorField, error) {
	...
	if err != nil {
		log.Warn("fail to parse dim", zap.String("dim", dimStr), zap.Error(err))
		return nil, err
	}
...
func (sd *shardDelegator) OptimizeSearchBasedOnClustering(req *querypb.SearchRequest, sealeds []SnapshotItem) (*querypb.SearchRequest, []SnapshotItem) {
	...
	if metricType != distance.L2 && metricType != distance.IP {
		log.Warn("skip OptimizeSearchBasedOnClustering as supported metric type not supported", zap.String("metricType", metricType))
		return req, sealeds
	}
	...
	for _, bSearchVector := range bSearchVectors {
		...
		for _, segment := range segments {
			if segment.ClusteringInfos == nil || segment.ClusteringInfos.GetVectorClusteringInfos() == nil {
			...
			log.Debug("distance between bSearchVector and cluster center",
				...
				zap.Any("clusterCentroid", vectorClusteringInfo.Centroid))
			if len(distance) > 0 {
