...
func (q *QuotaCenter) calculateReadRates() {
	...
	enableQueueProtection := Params.QuotaConfig.QueueProtectionEnabled
	// query latency
	...
	// read result
	enableResultProtection := Params.QuotaConfig.ResultProtectionEnabled
	...
	coolOff := func(realTimeSearchRate float64, realTimeQueryRate float64, collections ...int64) {
		...
		q.guaranteeMinRate(Params.QuotaConfig.DQLMinSearchRatePerCollection, internalpb.RateType_DQLSearch, collections...)
		q.guaranteeMinRate(Params.QuotaConfig.DQLMinQueryRatePerCollection, internalpb.RateType_DQLQuery, collections...)
...
func (q *QuotaCenter) getGrowingSegmentsSizeFactor() map[int64]float64 {
	...
	low := Params.QuotaConfig.GrowingSegmentsSizeLowWaterLevel
	high := Params.QuotaConfig.GrowingSegmentsSizeHighWaterLevel
...
func (q *QuotaCenter) resetCurrentRate(rt internalpb.RateType, collection int64) {
	...
	case internalpb.RateType_DMLInsert:
		q.currentRates[collection][rt] = Limit(Params.QuotaConfig.DMLMaxInsertRatePerCollection)
	case internalpb.RateType_DMLDelete:
		q.currentRates[collection][rt] = Limit(Params.QuotaConfig.DMLMaxDeleteRatePerCollection)
	case internalpb.RateType_DMLBulkLoad:
		q.currentRates[collection][rt] = Limit(Params.QuotaConfig.DMLMaxBulkLoadRatePerCollection)
	case internalpb.RateType_DQLSearch:
		q.currentRates[collection][rt] = Limit(Params.QuotaConfig.DQLMaxSearchRatePerCollection)
	case internalpb.RateType_DQLQuery:
		q.currentRates[collection][rt] = Limit(Params.QuotaConfig.DQLMaxQueryRatePerCollection)
	}
...
func (q *QuotaCenter) checkDiskQuota() {
	...
	collections := typeutil.NewUniqueSet()
	totalDiskQuota := Params.QuotaConfig.DiskQuota
